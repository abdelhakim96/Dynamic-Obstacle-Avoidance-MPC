...
class Obstacle():
    ...
    def step(self):
        ...
        self.x, self.vx, self.y, self.vy = self.predict_step(
            self.x, self.vx, self.y, self.vy, noise=True)
    ...
    def predict_step(self, x, vx, y, vy, noise=False):
        ...
        if self.random_move and noise:
            ...
            vx = min(max((1 + RANDOMNESS * noise_vx)
                     * vx, - V_MAX_OBST), V_MAX_OBST)
            vy = min(max((1 + RANDOMNESS * noise_vy)
                     * vy, - V_MAX_OBST), V_MAX_OBST)
...
class VisDynamicRobotEnv():
    def __init__(self, obstacles):
        ...
        self._ax.add_patch(plt.Circle((X_MIN + 2, Y_MIN + 2),
                           TOL, fill=False, edgecolor='orange'))
        self._ax.add_patch(plt.Circle((X_MAX - 2, Y_MAX - 2),
                           TOL, fill=False, edgecolor='g'))
    ...
    def _animate(self, t):
        ...
        self._traj_vis_pred.set_data(
            self._pred_traj[t, :, 0], self._pred_traj[t, :, 1])
